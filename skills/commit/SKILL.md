---
name: commit
description: Create commit messages following conventional commits conventions. Use when committing code changes, writing commit messages, or formatting git history. Follows conventional commits with issue references.
---
...
| `ci` | CI configuration |
| `chore` | Qualitative improvements that don't affect functionality (e.g. dependency updates, instrumentation) |
...
The scope should be the name of the core component or system affected, as perceived by someone reading a changelog — not the literal file changed. For example, if code primarily affects billing, use the `billing` scope even if the changes are in utility files or DB schema.

It is also acceptable to use the feature or project name as a scope.
